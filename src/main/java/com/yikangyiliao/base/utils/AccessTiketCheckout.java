...
public class AccessTiketCheckout {
	...
	public static Boolean checkAccessTiketLayout(String accessTiket, HttpServletRequest hsr) throws Exception {
		try {
			...
			if (null != accessTik) {
				...
				if (accessTik.contains("UD") && accessTik.contains("￥LDT") && accessTik.contains("￥MC")) {
					...
					if (ldtIndex > udIndex && mcIndex > ldtIndex) {
						...
						logger.error("ip为：" + NetworkUtil.getIpAddress(hsr) + "-->输入格式accessTiket 错误数据，内容为："
								+ accessTiket + " 这货，已经到了测试 参数 顺序这层，注意了。");
						throw new ExceptionInfo(ExceptionConstants.parameterException.accessTiketException.errorCode,
	 ...
	 * 
	 */
	public static String generateAccessTicket(String loginId, Long loginDateTime, String machineCode) throws Exception {
		return AES.Encrypt("UD" + loginId + "￥LDT" + loginDateTime + "￥MC" + machineCode, secretKey);
	}

	public static void main(String[] args) {
		try {
			System.out.println(AES.Decrypt(
					"ae7433528cdd54dc76e922c1d612c82dfcfc343a64e68012c34c1b74c4cd73dd5e4c7afd323d91954ba85f0a1bf9bb45",
					"yi99kang88yi99ka"));
		} catch (Exception e) {
