		...
		while (true) {
			try {
				...
				if (null != operationMessage) {
					...
					ForumPostsAnswer forumPostsAnswer = forumPostsAnswerManager
							.getForumpostsAnswerByAnswerId(forumPostAnswerId);
					...
					String subForumPostTitle = fp.getTitle().length() > 20 ? fp.getTitle().substring(0, 20)
							: fp.getTitle();
					String title = "";
					...
					if (fp.getForumPostGroup().equals(Byte.valueOf("1"))) {
						...
						title = "佳友 “" + createAnswerUserInfo.getUserName() + "” 评论了你的帖子 “"
								+ (fp.getContent().length() > 20 ? fp.getContent().substring(0, 20) : fp.getContent())
								+ "”";
					}
					// 添加 文章回答的 动态信息
					messageManager.insertDynamicFollowMessage(createAnswerUserInfo.getUserId(), fp.getCreateUserId(),
							title, title, fp.getForumPostId(), contentGroup);
					...
					if (null != UserConfigrationsCache.get(String.valueOf(fp.getCreateUserId()))) {
						UserConfigration userConfigration = (UserConfigration) UserConfigrationsCache
								.get(String.valueOf(fp.getCreateUserId()));
						if (userConfigration.getDynamicAlert() == 1) {
							try {
								...
								e.printStackTrace();
								LOG.error(LogUtils.getErrorStr(this.getClass().getName(), "run:90",
										"推送异常:" + e.getMessage()));
							}
						}
					}
				...
				e.printStackTrace();
				LOG.error(LogUtils.getErrorStr(this.getClass().getName(), "run:90", "forumPostAnswerMsg:" + e.getMessage()));
			}
		}
	}
